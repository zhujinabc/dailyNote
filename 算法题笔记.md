...
function len(s){//用滑动指针做，一个开始一个结束
    let map = new Map()
    let start = 0//定义左指针
    let res = 0
    for(let i=0;i<s.length;i++){
        if(map.has(s[i] && map.get(s[i])>=start)){
            start = map.get(s[i]) + 1//出现重复字符，将左指针移动到重复字符下一位
        }
        map.set(s[i],i)
...
function mostLength(s){
    let res = s[0] || ''
    for(let i=0;i<s.length;i++){
        for(let j =1;j<=2;j++){
            let left = i,rigth = i+j//这快也需要注意
            while(left>=0 && right<s.length && s[left] === s[right]){
                left --
                right ++
            }
            let length = right - left -1//这块需要注意
            if(length > res.length){//如果比当前的res大则替换
...
function maxArea(height){//双指针，一个头一个尾
    let i = 0
...
function isValid(s){
    if(s.lenght % 2 !== 0) return false//如果不是偶数那么肯定无效
    let stack = []
    let map = {')':'(',']':'[','}':'{'}
    for(let i of s){
        if(i==='(' || i==='[' || i==='{'){
